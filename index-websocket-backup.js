...
async function connectToBrowserless() {
  ...
  for (const endpoint of BROWSERLESS_ENDPOINTS) {
    try {
      ...
      const browser = await chromium.connect(wsEndpoint, { timeout: 25000 });
  ...
  throw lastError || new Error('Failed to connect to any Browserless endpoint');
}
...
app.get('/test-connection', async (req, res) => {
  ...
  try {
    ...
    console.log('Token configured:', !!BROWSERLESS_TOKEN);
    console.log('Token length:', BROWSERLESS_TOKEN?.length || 0);
    if (BROWSERLESS_TOKEN) {
    ...
    res.status(500).json({
      ...
      hint: 'Please verify your BROWSERLESS_TOKEN at https://www.browserless.io/dashboard'
    });
  }
});
